...
const content = {
  en: {
    ...
    footer: 'Company profile, custom application, and internal system development.',
    chapters: [
      {
        label: 'Future',
    ...
    about: {
      ...
      cards: [
        ['01', 'Workflow First', 'We study how your team actually works before deciding what to build.'],
        ['02', 'Custom System', 'Every module is shaped around real operations, roles, and business goals.'],
        ['03', 'Long-Term Fit', 'The system is designed to be maintainable, scalable, and useful after launch.'],
      ],
    },
    problems: {
    ...
    servicesTitle: {
      kicker: 'Capabilities',
      title: 'Everything needed to turn business operations into digital infrastructure.',
    },
    services: [
      ['01', 'Company Profile', 'Premium business websites with stronger positioning, clearer content, and responsive UI.'],
      ['02', 'Custom App', 'Web applications for approval flows, reporting, finance, inventory, CRM, or internal work.'],
      ['03', 'ERP Modules', 'Operational systems split into modules so they can grow with your team.'],
      ['04', 'Care & Redesign', 'Maintenance, UI cleanup, performance tuning, and feature iteration after launch.'],
    ],
    projectsHead: {
      kicker: 'Projects',
    ...
    projects: [
      {
        ...
        metrics: ['Quotation in minutes', 'Multi-level approval', 'Print-ready PDF output'],
      ...
      },
      {
        ...
        metrics: ['Request-to-payment flow', 'Role-based access', 'PDF invoice export'],
        tech: ['Laravel', 'MySQL', 'Blade', 'jQuery', 'REST API', 'VPS', 'Nginx'],
        scope: [
          'Request management',
          'Project management',
          'Invoice management',
      ...
      },
      {
        ...
        tech: ['React JS', 'Laravel', 'MySQL', 'Capacitor Android', 'Tesseract OCR', 'Ubuntu VPS', 'Nginx'],
        previewType: 'mobile',
        scope: [
  ...
  id: {
    ...
    footer: 'Pengembangan company profile, aplikasi custom, dan sistem internal bisnis.',
    chapters: [
      {
        label: 'Masa Depan',
        title: 'Kami membangun sistem digital untuk bisnis yang siap bergerak lebih cepat.',
      ...
      {
        label: 'Orbit',
        title: 'Setiap project dimulai dari memetakan gravitasi bisnis yang sebenarnya.',
      ...
      {
        label: 'Peluncuran',
        title: 'Dari website company profile sampai command center operasional.',
    ...
    about: {
      ...
      cards: [
        ['01', 'Workflow First', 'Kami memahami cara kerja tim terlebih dahulu sebelum menentukan sistem yang dibangun.'],
        ['02', 'Sistem Custom', 'Setiap modul disusun mengikuti operasional, role, dan tujuan bisnis yang nyata.'],
        ['03', 'Siap Bertumbuh', 'Sistem dibuat agar mudah dirawat, dikembangkan, dan tetap berguna setelah launch.'],
      ],
    },
    problems: {
    ...
    servicesTitle: {
      kicker: 'Kapabilitas',
      title: 'Semua yang dibutuhkan untuk mengubah operasional bisnis menjadi infrastruktur digital.',
    },
    services: [
      ['01', 'Company Profile', 'Website bisnis premium dengan positioning lebih kuat, konten jelas, dan UI responsive.'],
      ['02', 'Aplikasi Custom', 'Aplikasi web untuk approval, reporting, finance, inventory, CRM, atau kerja internal.'],
      ['03', 'Modul ERP', 'Sistem operasional berbasis modul agar mudah berkembang mengikuti kebutuhan tim.'],
      ['04', 'Care & Redesign', 'Maintenance, perapihan UI, optimasi performa, dan iterasi fitur setelah launch.'],
    ],
    projectsHead: {
      kicker: 'Project',
      title: 'Arsip misi',
    ...
    projects: [
      {
        ...
        metrics: ['Quotation selesai dalam menit', 'Approval multi-level', 'Output PDF siap cetak'],
        tech: ['Laravel', 'MySQL', 'Blade', 'jQuery', 'PHP'],
        scope: [
      ...
      },
      {
        ...
        metrics: ['Alur request sampai payment', 'Akses berbasis role', 'Export invoice PDF'],
        tech: ['Laravel', 'MySQL', 'Blade', 'jQuery', 'REST API', 'VPS', 'Nginx'],
        scope: [
          'Manajemen request',
          'Manajemen project',
...
function Header({ locale, onHome, onLocaleChange, onNavigate, t }) {
  ...
  return (
    <header className="site-header">
      <button className="brand reset-button" type="button" onClick={onHome} aria-label="ArcDev home">
        <img src={mark} alt="" />
        <span>ArcDev</span>
      </button>
      ...
      <nav className={menuOpen ? 'nav-links open' : 'nav-links'} aria-label="Primary navigation">
        {t.nav.map(([label, href]) => (
          <a key={label} href={href} onClick={(event) => onNavigate(event, href, closeMenu)}>
            {label}
          </a>
        ))}
      </nav>
      ...
      <div className="header-actions">
        ...
        <a className="header-cta" href="#contact" onClick={(event) => onNavigate(event, '#contact')}>
          {t.cta}
        </a>
      </div>
  ...
  return (
    <div className={previewClass}>
      ...
      <div className="preview-screen">
        ...
        {image ? (
          <img className="preview-image" src={image[0]} alt={`${project.title} ${image[1]}`} />
        ) : (
          <>
...
function HomePage({ onProjectOpen, t }) {
      ...
      <section className="about-section" id="about">
        <div className="about-layout">
          ...
          <div className="about-visual-grid">
            {t.about.cards.map(([number, title, text]) => (
      ...
      <section className="projects-section" id="projects">
        ...
        <div className="project-orbit-list">
          {t.projects.map((project, index) => (
            <button className="project-row" type="button" key={project.id} onClick={() => onProjectOpen(project.id)}>
...
function ProjectPage({ project, onBack, t }) {
  return (
    <main className={`page-shell project-detail ${project.previewType === 'mobile' ? 'mobile-project' : ''}`}>
      ...
      <section className="project-gallery" aria-label={`${project.title} ${t.detail.galleryLabel}`}>
...
function App() {
  ...
  const activeProject = activeProjectId ? t.projects.find((project) => project.id === activeProjectId) : null;
  ...
  return (
    <>
      <SpaceBackground />
      <Header locale={locale} onHome={closeProject} onLocaleChange={setLocale} onNavigate={navigateToSection} t={t} />
      <div className={`locale-scope locale-${locale}`}>
        {activeProject ? (
        <ProjectPage project={activeProject} onBack={closeProject} t={t} />
      ) : (
        <HomePage onProjectOpen={openProject} t={t} />
      )}
      </div>
      <footer>
        <span>ArcDev</span>
